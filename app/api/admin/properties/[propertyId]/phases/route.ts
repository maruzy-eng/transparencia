...
export async function POST(
  ...
  if (!admin) {
    return withSessionCookie(
      NextResponse.redirect(
        new URL(
          `/admin/login?next=${encodeURIComponent(`/admin/properties/${propertyId}/edit?tab=phases`)}`,
          request.url,
        ),
        303,
      ),
  ...
  if (!result.success) {
    return withSessionCookie(
      NextResponse.redirect(
        editPhasesUrl(request, propertyId, { error: result.error ?? "Erro ao salvar fase." }),
        303,
      ),
      request,
    );
